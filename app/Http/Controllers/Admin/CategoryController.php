...
class CategoryController extends Controller
{
    ...
    public function index(REQUEST $request) {
        $page_heading = "Interested Categories";
        $mode="List";
    ...
    public function create($id='') {
        ...
        if($id) {
            ...
            $page_heading = "Category";
            $id = decrypt($id);
            $category = Category::where(['category_id'=>$id])->get()->first();
        ...
        $parent_categories = Category::where(['parent_category_id'=>0])->get();

        return view('admin.category.create',compact('page_heading','mode','category_name','category_image','category_status','id','lang_items','parent_categories','parent_category_id','category_icon'));
    }
    ...
    public function submit(REQUEST $request){
        ...
        $o_data  = [];
        $errors  = [];
        $o_data['redirect'] = route('category.list');
        ...
        if ($validator->fails()) {
            ...
            $id = $request->id;
            $item = Category::where(['category_name'=>$request->category_name])->where('category_id','!=',$id)->get();
            ...
            if($item->count() > 0){
                ...
                if($id > 0){
                    DB::beginTransaction();
                    try{
                        $item = Category::where(['category_id'=>$id])->get()->first();
    ...
    public function getCategoryList(Request $request){
        ...
        $sqlBuilder = Category::select([
            ...
            DB::raw('categories.category_status::text as category_status'),
            DB::raw('categories.created_at::text as created_at'),
            DB::raw('categories.category_id::text as category_id')
        ])->leftJoin('categories as p_category',function($join){
            $join->on('p_category.category_id','=','categories.parent_category_id');
        });
    ...
    public function change_status(REQUEST $request,$id){
        ...
        if($item->count() > 0){
            $item=$item->first();
            Category::where('category_id','=',$id)->update(['category_status'=>$request->status]);
            $status = "1";
