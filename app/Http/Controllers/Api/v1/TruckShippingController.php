...
class TruckShippingController extends Controller
{
    ...
    public function ftl_booking_create(Request $request){
    ...
    // public function create_ftl_booking(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if(!empty($booking_cart)){
                ...
                if(!empty($booking)){
                    ...
                    BookingCart::where('device_cart_id',$request->device_cart_id)->where('deligate_type','ftl')->delete();
                    ...
                    // } else { 
                        ...
                        exec("php " . base_path() . "/artisan notify_driver_on_booking:user " . $booking->id . " > /dev/null 2>&1 & ");
                    //}

                    $status = "1";
                    $message = "Request Submitted Successfully";
                    $o_data['booking_id']   = $booking->booking_number; 
                    // $o_data['booking']   = Booking::with('booking_truck')->where('id',$booking->id)->first(); 
                }
                else{
        ...
        if ($validator->fails()) {
            ...
            if(!empty($booking_cart)){
                ...
                if(!empty($selected_trucks)){
                    ...
                    $status = "0";
                    $message = "Truck could not remove";
                }
            ...
            if(!empty($booking_cart)){
                ...
                if($request->has('parent_id') && $request->parent_id > 0 ){
                    BookingCart::where([
                        ...
                        'deligate_type'        => 'ltl',
                        'device_cart_id'       => $request->device_cart_id
                    ])->update(['booking_number' => $request->booking_number,'parent_id'=>$request->parent_id]);
                }
                ...
                CartDeligateDetail::updateOrCreate([
                   ...
                   'num_of_pallets'             => $request->num_of_pallets
                ]);
                ...
                $deligate_details = [
                    ...
                    'num_of_pallets'            => $booking_cart->cart_deligate_detail->num_of_pallets ?? ''
                ];
        ...
        if ($validator->fails()) {
            ...
            if(!empty($booking_cart)){
                ...
                if(!empty($booking)){
                    ...
                    $booking_deligate_detail->num_of_pallets        = $booking_cart->cart_deligate_detail->num_of_pallets;
                ...
                if(!empty($booking)){
                    ...
                    BookingCart::where('device_cart_id',$request->device_cart_id)->where('deligate_type','ltl')->delete();
    ...
    public function booking_change_address(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $booking_cart = BookingCart::where('sender_id',$user_id)->first();
            if(!empty($request->collection_address_id))
            {
            $booking_cart->collection_address = $request->collection_address_id;
            }
            if(!empty($request->deliver_address_id))
            {
            $booking_cart->deliver_address = $request->deliver_address_id;
            }
            $booking_cart->save();
                $status = "1";
