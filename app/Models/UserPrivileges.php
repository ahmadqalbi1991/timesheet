...
class UserPrivileges extends Model
{
    ...
    public static function privilege(){
        $data = \App\Models\UserPrivileges::where('designation_id',Auth::user()->designation_id)->pluck('privileges')->first();
        // $data = UserPrivileges::leftjoin('users', 'users.id', 'user_privileges.user_id')
