...
@section('header')
    ...
    .deligate_details {
            display:{{$booking->deligate_type == 'fcl'?'none':''}};
    }
            ...
            <div class="card-body">
                            ...
                            <form id="admin_form" action = "{{ route('bookings.update.trucking',['id' => $booking->id]) }}" method = "POST" enctype="multipart/form-data">
                                ...
                                <div class = "row">
                                    ...
                                    <div class="col-md-6"  id = "fcl-lcl">
                                        <label class="form-label">Type</label>
                                        <select class="form-control-plaintext jqv-input fcl-lcl-input" data-jqv-required="true" name="type" disabled>
                                            <option  value="">Select Type</option>
                                            <option  value="fcl" {{ $booking->deligate_type == 'fcl'?'selected':'' }}>FCL</option>
                                            <option  value="lcl" {{ $booking->deligate_type == 'lcl'?'selected':'' }}>LCL</option>
                                        </select>
                                ...
                                {{--  
                                 @php
                                        $truck = $booking->booking_truck->first();
                                    @endphp
                                    @if($truck && $truck->container)
                                        <div class="col-md-6 collection_address_div">
                                            <label class="form-label">Container Name</label>
                                            <input type = "text" class="form-control-plaintext deligate_details_input" name = "" disabled value = "{{ $truck->container->name ?? '' }}" data-jqv-required="true">
                                        </div>
                                        <div class="col-md-6 collection_address_div">
                                            <label class="form-label">Container Name</label>
                                            <input type = "text" class="form-control-plaintext deligate_details_input" name = "quantity"  value = "{{ $truck->quantity ?? '' }}" data-jqv-required="true">
                                        </div>
                                        <div class="col-md-6 collection_address_div">
                                            <label class="form-label">Container Name</label>
                                            <input type = "text" class="form-control-plaintext deligate_details_input" name = "gross_weight" value = "{{ $truck->gross_weight ?? '' }}" data-jqv-required="true">
                                        </div>
                                    @endif   
                                --}}    
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <select class="form-control-plaintext truck_type" id = "truck_type"  disabled>
                                                        ...
                                                        <option value = "{{$row->id}}" {{ $booking_truck->truck_id == $row->id?'selected':'' }}>
                                                            {{$row->name." -- "."(".$row->dimensions.")"." -- ".strtoupper($row->type)}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-2">
                                                ...
                                                <div class="col-md-2">
                                                    <label class="form-label">Total Gross Weight</label>
                                                    <input type = "text" class="form-control-plaintext" data-jqv-required="true" value = "{{ $booking_truck->gross_weight }}" disabled>
                                                </div>
                                                {{--
                                                <div class="col-md-2">
    ...
    $('body').on('submit', '#admin_form', function(e) {
        ...
        $(document).ready(function(){
            ...
            $(document).on('change','#deligate',function(){
                ...
                if($(this).val() == 1){
                    $('#fcl-lcl').show();
                    $('.fcl-lcl-input').prop('disabled',false);
                }else{
                    $('#fcl-lcl').hide();
                    $('.fcl-lcl-input').prop('disabled',true);
                }
