                    ...
                    <div class="col-md-12 col-sm-12 py-4">
                                ...
                                <div class = "row">
                                    ...
                                    <div class="col-md-6 deligate_details">
                                     ...
                                     <div class="col-md-6 deligate_details">
                                        ...
                                        <div class="form-control-plaintext" >
                                            {{ $booking->booking_deligate_detail->num_of_pallets ?? '' }}
                                        </div>
                                    </div>
                            ...
                            <div class = "row">
                               ...
                               <!--  <div class = "col-md-4">
                                    ...
                                    @if($booking->booking_accepted_qoutes->count() > 0)
                                    @forelse($booking->booking_accepted_qoutes as $accepted_qoute)
                                    @if($accepted_qoute->customer_signature != null && $accepted_qoute->customer_signature != 0)
                                        <img src = "{{ asset('storage/bookings/'.$accepted_qoute->customer_signature) }}" width = "200">
                                    @else
                            ...
                            <div class = "row">
                                <div class = "col-md-6">
                                    ...
                                    <table class = "table table-striped">
                                        ...
                                        <tr>
                                            @if(!in_array($tracking->quote_id,$quotear))
                                                <th>{{$tracking->driver->name}} </th>
                                            @else
                                                <th> </th>
                                            @endif
                                        ...
                                        @php $quotear[] = $tracking->quote_id;  @endphp
                                        @endforeach
                                    </table>
                                </div>
                                <div class = "col-md-6">
                                    ...
                                    @forelse($booking->booking_accepted_qoutes as $accepted_qoute)
                                        <div class="text-left">
                                                ...
                                                <a href = "{{ route('drivers.view',['id' => encrypt($accepted_qoute->driver->id)]) }}">
                                                    <h6 class="mt-0"><strong>{{ $accepted_qoute->driver->name ?? ''}}</strong></h6>
                                                    ...
                                                    <label class="form-label">Remark</label>
                                                    {{ $accepted_qoute->delivery_note??'' }}
                                                         </br>                                                  
                                                @if($accepted_qoute->status == 'delivered')  
                                                @if($accepted_qoute->customer_signature !=null)
                                                 <label class="form-label">Customer Signature</label>
                                                <img src = "{{ asset('storage/bookings/'.$accepted_qoute->customer_signature) }}" width = "200">
                                                @endif
                                                ...
                                                @endif
                                                @endif
                                                @if($accepted_qoute->driver_advance_amount > 0 ) 
                                                    <label class="form-label">Advance Amount</label>
                                                    <p>{{$accepted_qoute->driver_advance_amount}}</p>
                                                    
                                                 @endif

                                                @php $extraCHarges = \App\Models\BookingExtraPayment::where('accepted_quote_id',$accepted_qoute->id)->get();
                                                $charge = config('global.extra_charge');
                                                 @endphp
                                             
                                                @if($extraCHarges->count() > 0 )
                                                </br>
                                                </br>
                                                 <h7><strong>Extra Expenses</strong></h7></br>
                                                ...
                                                @foreach($extraCHarges as $ech)
                                                    <tr><td>{{$charge[$ech->extra_charge_id]}}  @if($ech->extra_charge_id == 4) {{'-'.$ech->extra_charge_name}} @endif</td>
                                                        <td>{{$ech->extra_charge_amount}}</td>
                                                    </tr>
                                                @endforeach
                                                </table>
                                                @endif
                                ...
                                <div class = "col-md-4">
                                    ...
                                    <table class = "table table-striped">
                                       ...
                                       <!--  <tr>
                                        ...
                                        @if($booking->grand_amount)
                                        <tr>
                                            <th>Grand Total: </th>
                                            <td>{{ number_format($booking->grand_amount,2)}} </td>
                                        </tr>
                                        @endif
                                         @php
                                            $extraAmount = \App\Models\BookingExtraPayment::where('booking_id',$booking->id)->sum('extra_charge_amount');
                                         @endphp
                                         <tr>
                                            <th>Extra Expenses: </th>
                                            <td>{{ number_format($extraAmount,2)}} </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ number_format($booking->grand_total - $booking->total_received_amount-$extraAmount,2)}} </td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
